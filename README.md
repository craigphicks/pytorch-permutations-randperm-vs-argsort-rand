...
This document will define a family of criteria $P_{\text{crit}}(K;m)$ that can be used to determine wether a particular length $K$ will have too many collisions, or not, as follows:

- if $P_{\text{crit}}(K; m) < C_{\text{thresh}}$ then $m$ is acceptable
- else use a larger $m$ or use `randperm`

where 
- $K$ is the permutation length, fixed with respect to the decision rule,
- $m \in \{24, 32, 64\}$ corresponds to \{\text{float}, \text{integer32}, \text{integer64}\}$,
...
- $P_{\text{crit}}(K; m)$ increases monotonically with $K$ and $m$.


The statistical measure $P_{\text{crit}}(K; m)$ is defined such that given a random permutation generated by either `randperm(K)` or `torch.argsort(torch.rand(K))` with a 50% chance of each method being used, but without knowing which method was used. $P_{\text{crit}}(K; m)$ is the probability of guessing correctly which distribution the sample was drawn from, using an optimal guessing strategy.
...
Section ["Best Guess Criterion: Single Set of K Tosses"](#best-guess-criterion-single) introduces the model for $P_{\text{crit}}(K; m)$ as described above.
...
First note that the $\text{cdf}(c)$ can be replace by the error function $\text{erf}$ with scaling:
...
Subtituting $c= \frac{\sqrt{K}(K-1)}{2^{m+2}}$ and approximating the $K-1$ term as $K$, 
...
The only difference with equation 9.2 is the final subtrahend $\frac{\log_2(N)}{3}$.

Repinting the the table above replacing $N=1$ with $N=2^16=65,356$ we get
...
For example, with $P_{\text{crit}} = 0.55$ and $m=24$, a permutation length of $K=32,268 = 2^{15}$ would no longer the critieria.  However, for a value of $m=32$ it would satisfy the criteria.

<a name="additional-considerations"></a>
## 7. Additional Considerations

### 7.1 $d^2$ is small for the solution $P_{\text{crit}}=0.55$
...
$$ for $x$, to get the x-axis location for point where the the fair and biased distribution are equal, as shown in [Figure 1](#figure-1).
...
### 7.2. Collisions of 3 or more are extremely rare in $P_{\text{crit}} \leq 0.55$
...
### 7.3. How the value $m=24$ was obtained for the float type
...
### 7.4. Alternative criteria could have been used

#### 7.4.1. RelativeVariance
...
#### 7.4.4. How these alternatives relate
...
However, our chosen function $P_{\text{crit}}$ alone has the intuitive statistical interpretation on the range $0.5$ tp $1$, making it the most suitable choice as the basis for the criteria.
