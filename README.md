...
This document will define a family of criteria $P_{\text{crit}}(K;m)$ that can be used to determine whether a particular length $K$ will have too many collisions, or not, as follows:
...
The only difference with equation 9.2 is the final subtrahend $\frac{\log_2(N)}{3}$.
 ...
 Replacing $N=1$ with $N=2^16=65,356$ in the above table we get:
...
For example, with $P_{\text{crit}} = 0.55$ and $m=24$, a permutation length of $K=32,268 = 2^{15}$ would no longer satisfy the criterion.  However, for a value of $m=32$ it would satisfy the criterion.
